...
# known-bad node IDs to exclude from --export even if reachable (TCP-reachable but
# misbehaving). 1d07c049... spams oversized p2p messages (8200 > 1034) -> reconnect
# storms. Extend with --deny <id,id,...>.
DENY = {"1d07c049908e614f5d00bf64539581178a2a7f0d"}
TIMEOUT = 3.0
...
def main():
    ...
    ap.add_argument("--deny", default="", help="extra node IDs to exclude (comma-separated), added to the built-in known-bad list")
    ap.add_argument("--timeout", type=float, default=TIMEOUT, help="dial timeout seconds (default 3)")
    args = ap.parse_args()
    TIMEOUT = args.timeout
    deny = set(DENY) | {x.strip() for x in args.deny.split(",") if x.strip()}
    ...
    if args.export:
        ...
        candidates = {c for c in candidates if parse(c)[0] not in deny}
        ...
        if n_excluded:
            print("# {} known-bad peer(s) excluded".format(n_excluded))
    ...
    for r in sorted(results, key=lambda r: (r["up"], r["ms"] if r["up"] else 0)):
        ...
        mark = "  [known-bad: exclude]" if r["id"] in deny else ""
        print("  {}  {:<22} {}...  {}{}".format(tag, r["host"] + ":" + r["port"], r["id"][:10], detail, mark))
